...
| **A5-refined** | **Always render; subscription-scoped quota + provider-scoped session** | The z.ai quota segment is *your plan's balance* — shown whenever a z.ai provider is configured, independent of the active provider (dimmed when z.ai ≠ active). The session segment reflects the active provider. No v1 per-provider `$ cost` (OR/Codex/Ollama deferred). **Never yields to native on provider switches** → no flicker when `/fleet` flips providers mid-session. Sole exception: an explicit `/statusline off` (user disable) restores the native footer by design; `/statusline on` re-installs mid-session without a restart. |
